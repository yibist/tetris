...
public class Block {
     ...
     *
     * @param type The type of block to be created. The valid types can be found in the {@link com.tetris.BlockType} enum list.
     * @param x    The x coordinate that the newly created block will be placed.
     * @param y    The y Coordinate that the newly created block will be placed.
     */
     ...
     *
     * @return returns new rotation of the block.
     */
    public int rotate() {
        for (Tile tile : tiles) {
            tile.x =  clockwiseRotationMatrix[0][0] * tile.x + clockwiseRotationMatrix[0][1] * tile.x;
            tile.y =  clockwiseRotationMatrix[1][0] * tile.y + clockwiseRotationMatrix[1][1] * tile.y;
        }
        rotation++;
        if (rotation < 0) {
            rotation = 3;
     ...
     *
     * @param pane The pane on which the Tiles will be drawn.
     */
    public void drawT(Pane pane) {
        for (Tile tile : tiles) {
            tile.drawTile(pane, this);
        }
    }
     ...
     *
     * @param direction The direction of movement.<br>
     *                  Valid inputs are:
     ...
     *
     * @param placedBlocks A collection of already placed Tiles to be used for determining if a tile on tile collision would take place.
     * @param limitY      The limit for the y coordinate to be used if no block collisions take place.
     * @param limitX      The limit for the x coordinate to be used if no block collisions take place.
